...
Later in the course, we will come back to this dataset and perform a analysis that allows us to assess the different research hypotheses for 
...
fluctuations of the size that we observe here could happen by random change if we carefully randomise, we may use simulations where we simulate the allocation of the fish to the different doses.
...
We can try to remediate these deviations by log-transforming the data. Indeed, data with heavy
right tails can often be normalized by applying this strategy

Fit a (log-)linear model by log transforming the response. Generate diagnostic plots.
...
Dojo fish that are exposed to a higher dose of the poison will have a $log_{10}$ survival time that is on average `r log.model$coef[2] %>% abs %>% round(.,2)` lower per gram of poison that was administered additionally (95% CI [`r confint(log.model) %>% abs %>% round(.,2)`])
.
This decrease in survival time is significant
on the 5% significance level (p<0.001).
...
Dojo fish that are exposed to a higher dose of the poison will have a geometric mean of the survival time that is factor `r round(10^log.model$coef[2],2)` (95% CI [`r round(10^(confint(log.model)[2,]),2)`]) per gram of poison that the received more than fish exposed to a lower dose. This decrease is very significant on the 5% significance level (p<0.001).
...
Our analysis seems
to suggest that fish in uncontaminated water would only survive for about
...
There is a very significant effect of the poison on survival of Dojo fish (p< 0.001). Dojo fish that are exposed to a higher dose of the poison will have a survival time that decrease on average with a factor `r log.model$coef[2] %>% abs %>% 10^. %>% round(.,2)` per gram of poison that is added (95% CI [`r confint(log.model)[2,] %>% abs %>% 10^. %>% round(.,2)`]).
