...
Suppose that researchers are ,mainly interested in studying the effect of poison dose on the survival of fish. 
They know however that the weight can also impact the survival and might also change the effect of the poison dose. 

In this tutorial session we will focus on Dojo fish and we will model the survival time in function of the dose and the weight of the fish.  
...
```{r}
# lm_Int <- lm(Surv_time %>% log2 ~ Dose+Weight+Dose:Weight, data = poison_dojo) # equivalent
lm_Int <- lm(Surv_time %>% log2 ~ Dose*Weight, data = poison) # * -> short notation
...
```{r}
lm_additive <- lm(Surv_time %>% log2 ~ Dose + Weight, data = poison) # * -> short notation
summary(lm_additive)
```
...
of dojofish 
(p-value = `r format(summary(lm_additive)$coefficients[2,4],digits=3)`). 
The geometric average of the survival time for dojofish that are exposed to a poison dose that is 1mg/L larger is approximately
halfed, 
