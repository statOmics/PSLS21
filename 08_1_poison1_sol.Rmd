...
`r lm_simple$coef[2] %>% 2^. %>% round(.,3)` per gram of poison that 
is added (95% CI [`r confint(lm_simple)[2,] %>% 2^. %>% round(.,3)`]).
...
poison dose that is 1mg/L larger is approximately halved, 
...
dose. A interaction between weight and dose implies that the dose effect on 
