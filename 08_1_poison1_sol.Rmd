...
1. We will first analyse the survival data by only considering the dose 
...
`r lm_simple$coef[2] %>% abs %>% 10^. %>% round(.,2)` per gram of poison that 
is added (95% CI [`r confint(lm_simple)[2,] %>% abs %>% 10^. %>% round(.,2)`]).
...
The QQ-plot suggest that there might be some deviation from normality in the left tail of the
...
We see that the effect of dose on survival time remains similar, however, it has become more significant 
after we have incorporated weight in our model. Indeed, from the data exploration, we learned that weight is associated with 
...
Therefore, we can estimate the effect between dose and survival time better while accounting for the weight.
...
of dojofish 
(p-value = `r format(summary(lm_additive)$coefficients[2,4],digits=3)`). The geometric average of the survival time for dojofish that are exposed to a poison dose that is 1mg/L larger is approximately
halved, 
factor = $2^{\beta_d}=$ `r format(2^(lm_additive$coef["Dose"]),digits=3)`) .
...
geometric average of the survival time of a dojofish that weighs 1 gram more than another dojofish is approximately twice as long 
...
dose. A interaction between weight and dose implies that the dose effect on the survival time changes according to the weight of the fish.
