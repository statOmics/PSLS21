...
```{r}
diet <- read.table("https://raw.githubusercontent.com/statOmics/PSLS21/data/dietRats.txt",header=TRUE)
head(diet)
```
...
Boxplot of the weight gain against protein source.

```{r}
diet %>% 
  ggplot(aes(x=protSource, y=weightGain, fill=protSource)) + 
...
The increase in the weight of the rats seems to depend on the protein source
received in the diet.

Boxplot of the weight gain against protein level.

```{r}
diet %>% 
  ggplot(aes(x=protLevel, y=weightGain, fill=protLevel)) + 
    scale_fill_brewer(palette="RdGy") +
    theme_bw() +
    geom_boxplot(outlier.shape=NA) + 
    geom_jitter(width = 0.2) +
    ggtitle("Weight gain dependence on protein level") +
    ylab("Weigth gain (g)") + 
    stat_summary(fun = mean, geom="point", shape=5, size=3, color="black", fill="black")
```
...
Boxplot of the weight gain against a factor that combines protein source and
protein level.

```{r}
diet %>% 
  mutate(combination = protSource:protLevel) %>%
  ggplot(aes(x=combination, y=weightGain, fill=combination)) + 
    scale_fill_brewer(palette="RdGy") +
    theme_bw() +
    geom_boxplot(outlier.shape=NA) + 
    geom_jitter(width = 0.2) +
    ggtitle("Weight gain dependence on protein source and protein level") +
    ylab("Weigth gain (g)") + 
    stat_summary(fun = mean, geom="point", shape=5, size=3, color="black", fill="black")
```
...
than in the chicken diet.

There is one extra layer of info in the data. Prior to the experiment,
the rats were divided in 10 homogeneous groups of 6 rats based on 
characteristics such as initial weight, appetite, etc. As such, rats coming
from the same `block` can be expected to be more similar than other rats.

```{r}
diet %>% 
  mutate(combination = protSource:protLevel) %>%
  ggplot(aes(x=combination, y=weightGain, col=combination)) + 
    scale_fill_brewer(palette="RdGy") +
    theme_bw() +
    geom_point(size=2) + 
    geom_line(aes(group = block),col="black") +
    ggtitle("Weight gain dependence on protein source and protein level") +
    ylab("weigth gain (g)")
```
...
The first assumption is met if we included the blocking factor as an additive
effect in the linear model. The other three assumptions can be assessed by
...
- The parameter $\beta_c$: the average weight difference between cereal-low and 
beef-low diet is `r lm1$coef["protSourcec"]`g.

- The parameter $\beta_p$: the average weight difference between pork-low and 
beef-low diet is `r lm1$coef["protSourcep"]`g.

- The parameter $\beta_h$: the average weight difference between beef-high and 
beef-Low diet is `r lm1$coef["protLevelh"]`g.

- The parameter $\beta_{ch}$ indicates the additional average weight increase 
due to the high protein level as compared to the low protein level for diets 
with a cereal protein source as compared to the weight increase that occurs 
in the reference class (here beef diet). Here this is negative, 
i.e. `r lm1$coef["protSourcec:protLevelh"]`g, thus the average weight gain 
for the cereal protein source increases less between high and low protein diets 
than for beef based protein sources. 

- The parameter $\beta_{ch}$ indicates the additional average weight increase 
due to the high protein level as compared to the low protein level for diets 
with a cereal protein source as compared to the weight increase that occurs in 
the reference class (here beef diet). Here this is negative, 
i.e.`r lm1$coef["protSourcep:protLevelh"]`, thus the average weight gain for 
the pork protein source increases slightly less between high and low protein 
diets than for beef based protein sources. 
...
interaction) on the weight gain of rats.
...
3. protein level for beef diets ($\mu_{b,h}-\mu_{b,l}=\beta_h$), 
...
average 17.1 and 16.7 g lower than in the high protein beef and pork diet, 
respectively (95% CI [6.4g,27.8g] and [6.0g,27.4g]) and the differences are 
extremely significant (p<0.001). The average weight gains after meat based 
diets at high protein levels, however, do not vary significantly 
(p>0.99, 95% CI [-11.1g,10.3g]).
...
The weight gain on average increases with 20.0g and 18.3g in the high protein 
level as compared to the low protein beef and pork diet, respectively 
(95%CI [9.3,30.7] and [7.6g,29.0g]). The protein level effect is not significant
